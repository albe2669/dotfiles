final: prev: let
  ...
  # Checksums from https://github.com/NixOS/nixpkgs/pull/511117
  checksums = {
...
in {
  # 2.1.114+ switched to a native binary architecture — the npm source build no longer works.
  # Override claude-code to use the pre-built binary approach (same as claude-code-bin).
